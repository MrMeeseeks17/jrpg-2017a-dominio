...
extends Entidad implements Peleable, Serializable {
	...
	/**
	 * habilidadesRaza Contiene las habilidades de la raza a la
	 ...
	 * @return String[] Devuelvelas habilidades de la raza a
	 * la que pertenece el personaje.
	 */
	 ...
	 * @return String[] Devuelve las habilidades de la casta
	 ...
	 */
	public Personaje(final String nombre2, final Casta casta2,
			final int id2, final int incrementoSalud,
			final int incrementoEnergia, final String nombRaza) {
		super(nombre2, 1, 10);
		this.casta = casta2;
		this.idPersonaje = id2;
		this.experiencia = 0;
		this.inteligencia = 10 + casta.bonusInteligencia();
		this.destreza = 10 + casta.bonusDestreza();
		this.incrementarFuerza(casta.bonusFuerza());
		x = 0;
		y = 0;
		saludTope = 100 + incrementoSalud;
		energiaTope = 100 + incrementoEnergia;
		//this.setSalud(saludTope);
		this.incrementarSalud(saludTope - this.getSalud());
		this.serEnergizado(energiaTope);
		nombreRaza = nombRaza;
		ataque = this.calcularPuntosDeAtaque();
		//setDefensa(this.calcularPuntosDeDefensa());
		this.incrementarDefensa(this.calcularPuntosDeDefensa());
	 ...
	 * @param nombRaza String con el nombre de la Raza
	 */
...
// + cast.bonusFuerza()
		...
		this.idPersonaje = id;
		//setDefensa(this.calcularPuntosDeDefensa());
		this.incrementarDefensa(calcularPuntosDeDefensa());
	 ...
	 */
	public void disminuirEnergia(final int energia2) {
	 ...
	 */
	public void incrementarDestreza(final int destreza2) {
	 ...
	 */
	public void incrementarInteligencia(final int inteligencia2) {
		this.inteligencia += inteligencia2;
	}

	/**
	 * Getter de Casta.
	 * @return Casta Devuelve la clasificación a la
	 * que pertenece el personaje
	 */
	 ...
	 */
	public void incrementarExperiencia(final int experiencia2) {
	 ...
	 */
	public void incrementarSaludTope(final int saludTope2) {
	 ...
	 */
	public void incrementarEnergiaTope(final int energiaTope2) {
	...
	/**
	 * Método golpeCritico: Calcula el daño crítico que hará el personaje.
	 * El cálculo se realiza a partir de la casta y fuerza de ataque
	 * del personaje.
	...
	/**
	 * Método puedeAtacar: Verifica si tiene la energía
	 * suficiente para atacar.
	 * @return True si la energía es mayor a la mínima necesaria, de lo
	...
	/**
	 * Método calcularPuntosDeAtaque: calcula el ataque según los puntos de
	 * fuerza.
	...
	public void restablecerSalud() {
		this.incrementarSalud(this.saludTope - this.getSalud());
		//setSalud(this.saludTope);
	}

	/**
	 * Método restablecerEnergia:
	 * restaura la energia del personaje a su estado inicial.
	 */
	...
	/**
	 * Método modificarAtributos: Modifica los puntos de
	 * ataque según la fuerza, defensa según la destreza
	 * y magia según la inteligencia del personaje.
	 */
	...
	@Override
	/**
	 * Metodo serAtacado: Calcula el daño que recibe el Objeto que llama al
	 * metodo.
		...
		if (this.getRandom().nextDouble()
			...
			if (dañoRecibido > 0) {
				if (getSalud() <= dañoRecibido) {
					...
					this.incrementarSalud(-this.getSalud());
					//setSalud(0);
				} else {
	...
	/**
	 * Método serRobadoSalud: Calcula la cantidad de puntos
	 * de salud que le son quitados al Personaje atacado
	 * según el daño infligido. Si la cantidad de daño es
	 * mayor a los puntos de salud, el daño real será
	 * igual a los mismos.
	...
	/**
	 * Método serDesernegizado: Calcula la cantidad de energía
	 * que se le quita a un Personaje según la cantidad de
	 * daño infligido.
	 * @param daño Valor de daño infligido.
	 * @return Valor de energía quitada si el daño supera a
	 * la defensa. Si eldaño también supera la energía,
	 * entonces el valor del daño hecho es igual a la
	 * energía poseída y la energía queda en 0.
	 */
	...
	/**
	 * Método serCurado: Incrementa la salud del Personaje según el valor
	 * recibido, teniendo en cuenta el tope de puntos de salud.
	...
	/**
	 * Método serEnergizado: Incrementa la energía del
	 * Personaje según el valor recibido, teniendo en
	 * cuenta el tope de puntos de energía.
	...
	/**
	 * Método crearAlianza: Crea una alianza nueva y agrega
	 * al Personaje como miembro.
	...
	/**
	 * Método aliar: Alía dos personajes si estos no tienen alianza,
	 * el llamador y el que recibe por parámetro.
	 * @param nuevoAliado Recibe el Personaje que se agrega a la Alianza.
	 * @return True si ambos Personajes no tenía ninguna Alianza y se pudo
	 *         agregar realizar la misma. False si algún personaje ya tenía
	 *         Alianza.
	 */
	...
	/**
	 * Método asignarPuntosSkills: Incrementa los puntos de fuerza,
	 * destreza e inteligencia mientras no superen los 200 puntos.
	 ...
	 */
	public void AsignarPuntosSkills(final int fuerza,
			final int dest, final int intel) {
		if (this.getFuerza() + fuerza <= 200) {
			this.incrementarFuerza(fuerza);
		}
	...
	/**
	 * Método subirNivel: Sube el nivel del personaje mientras
	 * no sea el nivel máximo y si la experiencia del personaje
	 * es suficiente para avanzar al siguiente nivel en la tabla
	 * de niveles. Además se modifican los atributos por nivel y
	 * se incrementan los topes de salud y energía. Se resta la
	 * experiencia usada para subir de nivel.
	 */
	public void subirNivel() {
		...
		while (this.getNivel() != 100
				&& (this.experiencia >= Personaje.tablaDeNiveles
						[this.getNivel() + 1]
						+ acumuladorExperiencia)) {
			acumuladorExperiencia +=
				Personaje.tablaDeNiveles[this.getNivel() + 1];
			//setNivel(this.getNivel() + 1);
			this.incrementarNivel(1);
			this.modificarAtributos();
			this.saludTope += 25;
			this.energiaTope += 20;
		}
	...
	/**
	 * Método ganarExperiencia: Incrementa la experiencia según el valor
	 * recibido y sube de nivel si la experiencia incrementada es
	 * mayor o igual a la necesaria para estar en el siguiente nivel.
	...
	public boolean ganarExperiencia(final int exp) {
		...
		if (experiencia >= Personaje.
				tablaDeNiveles[this.getNivel() + 1]) {
			subirNivel();
			//this.incrementarNivel(1);
			return true;
		}
	...
	/**
	 * Método otorgarExp: Otorga una experiencia equvalente a su nivel
	 * multiplicado por 40.
	 ...
	 * @return True si hay posibilidad de ocurrencia,
	...
	/**
	 * Método habilidadRaza1: Habilidad 1 implementada por la raza del
	 * Personaje.
	 * @param atacado Objeto peleable al que se ataca.
	 * @return boolean
	 */
	...
	/**
	 * Método habilidadRaza2: Habilidad 2 implementada por la raza del
	 * Personaje.
	 * @param atacado Objeto peleable al que se ataca.
	 * @return boolean
	 */
	...
	public final int[] serRobadoPorHechicero(final Personaje caster) {
		int energiaRobada = this.serDesernegizado(
				caster.calcularPuntosDeMagia());
		int saludRobada = this.serRobadoSalud(
