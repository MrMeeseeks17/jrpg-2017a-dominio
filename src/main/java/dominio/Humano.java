...
public class Humano extends Personaje {
	...
	@Override
	/**
	 * Primera habilidad del personaje "Incentivar":
	 * Es una habilidad de apoyo, en caso de que el
	 * invocador poseea energía, la disminuye en 10 (el costo
	 * de realizar la habilidad) luego obtiene el atributo ataque del aliado
	 * objetivo y lo incrementa con sus puntos de magia aumentando
	 * el daño del objetivo.
	 */
	 ...
	 * invocador poseea energía, ataca con tantos puntos de
	 * ataque como la mitad de la salud del objetivo y en caso
	 * de causarle daño disminuye la energía del caster a la mitad
	 * si no le resta los 10 puntos de costo tradicionales.*/
	public boolean habilidadRaza2(final Peleable atacado) {
		if (this.getEnergia() > 10) {
			if (atacado.serAtacado(atacado.getSalud() / 2) > 0) {
				//this.setEnergia(this.getEnergia() / 2);
				int energActual=this.getEnergia();
				int enerPerdida= energActual-(energActual/2);
				this.disminuirEnergia(enerPerdida);
				return true;
			}
			this.disminuirEnergia(10);
		}
